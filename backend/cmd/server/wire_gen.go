...
func initializeApplication(buildInfo handler.BuildInfo) (*Application, error) {
	...
	channelMonitorService := service.ProvideChannelMonitorService(channelMonitorRepository, secretEncryptor, usageLogRepository, settingService)
	...
	modelPricingHandler := admin.NewModelPricingHandler(pricingService, settingRepository, channelService)
	...
	astrBotTokenRepository := repository.NewAstrBotTokenRepository(db)
	astrBotOperationRepository := repository.NewAstrBotOperationRepository(db)
	astrBotTokenService := service.NewAstrBotTokenServiceWithEncryptor(astrBotTokenRepository, userRepository, secretEncryptor, configConfig.Totp.EncryptionKeyConfigured)
	astrBotOperationService := service.NewAstrBotOperationService(astrBotOperationRepository)
	accountService := service.NewAccountService(accountRepository, groupRepository)
	idempotencyCoordinator := service.ProvideIdempotencyCoordinator(idempotencyRepository, configConfig)
	astrBotHandler := handler.NewAstrBotHandler(astrBotTokenService, adminService, accountService, channelService, usageService, pricingService, idempotencyCoordinator, auditLogService, astrBotOperationService, opsService)
	idempotencyCleanupService := service.ProvideIdempotencyCleanupService(idempotencyRepository, configConfig)
	handlers := handler.ProvideHandlers(authHandler, userHandler, apiKeyHandler, usageHandler, redeemHandler, subscriptionHandler, announcementHandler, channelMonitorUserHandler, channelMonitorV2Handler, adminHandlers, gatewayHandler, openAIGatewayHandler, handlerSettingHandler, totpHandler, passkeyHandler, handlerPaymentHandler, paymentWebhookHandler, availableChannelHandler, modelPlazaHandler, asyncImageHandler, batchImageHandler, astrBotHandler, idempotencyCoordinator, idempotencyCleanupService)
	...
	astrBotAuthMiddleware := middleware.NewAstrBotAuthMiddleware(astrBotTokenService, userService)
	astrBotSignatureMiddleware := middleware.NewAstrBotRequestSignatureMiddleware(astrBotTokenService)
	astrBotRateLimitMiddleware := middleware.NewAstrBotRateLimitMiddleware(redisClient)
	astrBotReadScopeMiddleware := middleware.ProvideAstrBotReadScopeMiddleware()
	astrBotWriteScopeMiddleware := middleware.ProvideAstrBotWriteScopeMiddleware()
	engine := server.ProvideRouter(configConfig, handlers, jwtAuthMiddleware, optionalJWTAuthMiddleware, adminAuthMiddleware, apiKeyAuthMiddleware, auditLogMiddleware, stepUpAuthMiddleware, astrBotAuthMiddleware, astrBotSignatureMiddleware, astrBotRateLimitMiddleware, astrBotReadScopeMiddleware, astrBotWriteScopeMiddleware, apiKeyService, subscriptionService, opsService, settingService, compositeRouteResolver, redisClient)
