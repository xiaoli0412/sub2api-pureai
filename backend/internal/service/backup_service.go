...
func (s *BackupService) deleteBackupData(ctx context.Context, record *BackupRecord) error {
	...
	if err := s.deleteBackupObjects(ctx, record); err != nil {
		return fmt.Errorf("delete S3 backup objects: %w", err)
	}
	return nil
}
...
func (s *BackupService) cleanupOldBackups(ctx context.Context, schedule *BackupScheduleConfig) error {
	...
	for _, r := range toDelete {
		if err := s.deleteBackupData(ctx, &r); err != nil {
			...
			toKeep = append(toKeep, r)
			if r.StorageType == BackupStorageTypeLocal {
				// 本地清理延续旧行为：失败只保留记录并记录日志，避免一次本地文件
				// 删除失败阻断其他过期记录的清理任务。
				logger.LegacyPrintf("service.backup", "[Backup] 删除本地过期备份失败 id=%s: %v", r.ID, err)
			} else {
