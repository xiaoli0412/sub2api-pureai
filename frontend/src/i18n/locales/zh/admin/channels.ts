export default {
    ...
    riskControl: {
      ...
      keywordImportTitle: '导入关键词 TXT',
      keywordImportHint: '上传 UTF-8 文本文件。以 [分类] 开头的行用于预览分组；空行和 # 注释会忽略。分类名称支持中文、英文或中英混合。',
      keywordImportChooseFile: '选择 TXT 文件',
      keywordImportNoFile: '尚未选择文件',
      keywordImportFileLimit: '仅支持 TXT · 最大 {size}',
      keywordImportApiOnly: '当前为「仅 API」策略：关键词列表不会参与判断，因此已禁用 TXT 导入。请切换为「仅关键词」或「关键词 + API」后再导入。',
      keywordImportPreview: '导入预览',
      keywordImportSummary: '共 {count} 个关键词，分为 {categories} 个分类 · 已跳过 {duplicates} 个重复行',
      keywordImportMode: '导入方式',
      keywordImportAppend: '追加',
      keywordImportReplace: '替换',
      keywordImportCategoryCount: '{count} 个关键词',
      keywordImportResultCount: '导入后：{count} 个不重复关键词（上限 {max}）',
      keywordImportTooLong: '其中 {count} 行超过 {max} 个字符，修正后才能导入。',
      keywordImportApply: '应用导入结果',
      keywordImportApplied: '已将 {count} 个关键词写入编辑器',
      keywordImportUncategorized: '未分类',
      keywordImportTextOnly: '请选择 .txt 文件。',
      keywordImportFileTooLarge: 'TXT 文件超过 {size}。',
      keywordImportEmpty: '文件中没有可导入的关键词行。',
      keywordImportReadFailed: '无法读取 TXT 文件。',
      blockedKeywordsTooMany: '最多保存 {max} 个不重复关键词。',
      blockedKeywordTooLong: '单个关键词不能超过 {max} 个字符。',
