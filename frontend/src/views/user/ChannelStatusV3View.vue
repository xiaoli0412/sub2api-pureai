<!--
  ...
  V3 reads the passive aggregation pipeline through the existing
  /channel-monitor-v2 endpoints with group_by=platform_group, so the backend's
  operator scope and the viewer's own group scope are applied server-side and
  unrelated groups can never appear here.
-->
<template>
  <AppLayout>
...
// Rows without a group_id are platform-level placeholders and are dropped, since
// V3 is a per-group console.
